...
/**
 * RTCM3 Message Type 1041 - IRNSS Ephemeris Data
 * 
 * @author goGPS Project
 */
public class Decode1041Msg implements Decode {

	public Decode1041Msg() {
	}
	...
	public Object decode(boolean[] bits, int week) {
		...
		// Toe (Time of Ephemeris): 32 bits (IRNSS seconds of week)
		...
		// ToC (Time of Clock): 32 bits (IRNSS seconds of week)
		...
		// TGD L5: 16 bits, scale 2^-35 seconds
		double tgdL5 = decodeSigned(bits, i, 16) * Math.pow(2, -35);
		eph.setTgdL5(tgdL5);
		i += 16;
		
		// TGD S-band: 16 bits, scale 2^-35 seconds
		double tgdS = decodeSigned(bits, i, 16) * Math.pow(2, -35);
		eph.setTgdS(tgdS);
		i += 16;
